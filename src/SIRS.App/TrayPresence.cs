...
/// <summary>
/// Keeps SIRS in the notification area while it is minimised (I4), with the icon colour showing
...
public sealed class TrayPresence : IDisposable
{
    ...
    public TrayPresence(Window window, Func<StreamState> stateProvider, Action toggleBroadcast)
    {
        ...
        menu.Items.Add("Open SIRS", null, (_, _) => ShowWindow());
    ...
    public void Update()
    {
        ...
        _lastState = state;
        _notifyIcon.Text = Truncate($"SIRS — {state.Headline()}");
