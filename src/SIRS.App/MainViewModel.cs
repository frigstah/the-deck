...
public sealed class MainViewModel : ObservableObject, IDisposable, IControlSurface
{
    ...
    public string? InputSourceHint => IsLoopbackInput
        ? "SIRS is broadcasting everything this PC plays through this device — music apps, browser tabs, everything. Sound stopping here means silence on air."
        : null;
    ...
    /// in SIRS, and the whole argument for the program is that the first screen does not look like
        ...
        set
        {
            try
            {
                if (value)
                {
                    ...
                    if (device is null)
                    {
                        StatusMessage = "SIRS could not find a second source to add.";
                        return;
                    }
    ...
    public string MetadataEndpointStatus
    {
        get
        {
            ...
            if (!server.IsRunning) return "Off. Turn it on to let your playout software send titles to SIRS.";
    ...
    public RelayCommand CopyEndpointUrlCommand => _copyEndpointUrlCommand ??= new RelayCommand(() =>
    {
        ...
        {
            StatusMessage = $"SIRS could not copy that: {ex.Message}";
        }
    });
    ...
    public void ToggleRecording()
    {
        try
        {
            ...
            else
            {
                ...
                var station = _selectedServer?.StationName is { Length: > 0 } name ? name : "SIRS";
    ...
    ControlResult IControlSurface.SetTitle(string title) => OnUiBlocking(() =>
    {
        ...
        // for the title needs to be told, or it will look like SIRS is dropping updates.
        if (UsingTextFileMetadata && _engine.NowPlaying.Source != MetadataSource.Remote)
        {
            return ControlResult.Refused(
                "SIRS is taking titles from somewhere else at the moment, so this one was not used.");
        }
    ...
    ControlResult IControlSurface.StartRecording() => OnUiBlocking(() =>
    {
        ...
        return IsRecording
            ...
            : ControlResult.Refused(string.IsNullOrWhiteSpace(StatusMessage)
                ? "SIRS could not start recording."
                : StatusMessage);
    });
    ...
    ControlResult IControlSurface.SetGainDb(double db) => OnUiBlocking(() =>
    {
        ...
        return Math.Abs(clamped - db) < 0.001
            ...
            : ControlResult.Done($"Input level set to {clamped:0.0} dB, the nearest SIRS allows.");
    });
    ...
    public string ControlEndpointStatus
    {
        get
        {
            ...
            {
                return "Off. Turn it on to let other programs, or SIRS's own command line, drive SIRS.";
            }

            var reach = _control.AllowOtherComputers
                ? "Anything on your network can control SIRS with the password."
                : "This computer only.";
        ...
        get
        {
            ...
            if (!_midiInput.IsRunning)
            {
                return MidiDevices.Count == 0
                    ? "No MIDI controllers found. Plug one in and reopen SIRS."
    ...
    public RelayCommand OpenReleasePageCommand => _openReleaseCommand ??= new RelayCommand(() =>
    {
        ...
        {
            StatusMessage = $"SIRS could not open that page: {ex.Message}";
        }
    });
    ...
    public string? WaitingForDevice => _engine.IsWaitingForDevice
        ? "Waiting for the audio input to come back. SIRS will pick it up on its own as soon as it does."
        : null;
    ...
    public void Dispose()
    {
        ...
        // SIRS --live would aim at a port nobody is listening on.
        _control.Dispose();
        _midiInput.Dispose();
