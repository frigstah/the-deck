...
/// which of the two they quote. Rather than making the user find out, SIRS tries the port they
...
public sealed class ShoutcastSink(ServerProfile profile, EncoderSettings encoder) : IStreamSink
{
    ...
    public async Task ConnectAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var port in new[] { _profile.Port, _profile.Port + 1 })
        {
            ...
            try
            {
                ...
                {
                    ConnectionNote =
                        $"Connected on port {port}. SHOUTcast takes broadcasts on the port after the one listeners use, so SIRS moved up from {_profile.Port} for you.";
                }
    ...
    public async Task UpdateMetadataAsync(string title, CancellationToken cancellationToken)
    {
        ...
        request.Headers.UserAgent.ParseAdd("SIRS/1.0 (Mozilla Compatible)");
