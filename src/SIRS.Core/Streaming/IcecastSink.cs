...
public sealed class IcecastSink(ServerProfile profile, EncoderSettings encoder) : IStreamSink
{
        ...
        if (string.IsNullOrWhiteSpace(response))
        {
            ...
            if (string.IsNullOrWhiteSpace(response))
            {
                ...
                throw new StreamException(
                    ...
                    $"{_profile.Host} accepted the connection but did not answer. It may not be an Icecast server — open the server settings and press Test to let SIRS check.");
            }
    ...
    private string BuildRequest(bool useLegacySourceMethod)
    {
        ...
        builder.Append("User-Agent: SIRS/1.0\r\n");
    ...
    private void Interpret(string response)
    {
        ...
        switch (status)
        {
            ...
            case null:
                throw new StreamException(
                    StreamFailure.Protocol,
                    $"{_profile.Host} sent an answer SIRS did not understand. It may not be an Icecast server.");
    ...
    public async Task UpdateMetadataAsync(string title, CancellationToken cancellationToken)
    {
        ...
        request.Headers.UserAgent.ParseAdd("SIRS/1.0");
