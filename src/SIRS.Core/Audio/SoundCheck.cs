    ...
    double Seconds)
{
    ...
    public string Detail => Advice switch
    {
        LevelAdvice.NoSignal =>
            "SIRS did not hear anything at all. Check you picked the right input and that it is not muted in Windows.",
        LevelAdvice.Good =>
    ...
    public void Play(string? outputDeviceId)
    {
        ...
        var device = AudioDevices.ResolveOrDefault(outputDeviceId, AudioDeviceKind.Output)
            ?? throw new AudioDeviceUnavailableException(
                "SIRS could not find anything to play through. Check your speakers or headphones are connected.");
