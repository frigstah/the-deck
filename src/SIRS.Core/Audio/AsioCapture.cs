...
/// in how audio is fetched, not in what SIRS then does with it, and threading a second capture kind
...
public sealed class AsioCapture : IWaveIn
{
    ...
    /// SIRS stores, but a prefix means a saved setting can never be mistaken for a WASAPI endpoint
    /// id - which would send SIRS looking for a device that does not exist.
    /// </summary>
    ...
    public void StartRecording()
    {
        ...
        // hung one must not hang SIRS along with it.
    ...
    private void DriverLoop()
    {
        ...
        catch (Exception ex)
        {
            _openFailure = new AudioDeviceUnavailableException(
                $"SIRS could not open the ASIO driver \"{_driverName}\". " +
        ...
        try
        {
            ...
            // panel - and since the resampler downstream is built from the rate SIRS believes it
            ...
            if (!asio.IsSampleRateSupported(_requestedSampleRate))
            {
                throw new AudioDeviceUnavailableException(
                    ...
                    "quality setting in SIRS that matches the rate it is already on.");
            }

            // Null playback: SIRS only ever records. Handing the driver a playback provider it would
        ...
        catch (Exception ex)
        {
            ...
            _openFailure = ex as AudioDeviceUnavailableException ?? new AudioDeviceUnavailableException(
                $"SIRS could not start recording from \"{_driverName}\": {ex.Message}");
