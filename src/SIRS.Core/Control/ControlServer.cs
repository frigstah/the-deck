...
/// easily POST one. A rule that makes SIRS unusable from the software it is meant to serve is the
...
public sealed class ControlServer : IDisposable
{
    ...
    public ControlServer(IControlSurface surface)
    {
        ...
        _server = new TinyHttpServer(Handle)
        {
            OpenWithoutTokenProblem =
                "Letting other computers control SIRS needs a password. Without one, anything on " +
    ...
    private (int Status, string Body) Run(string name, Func<ControlResult> action)
    {
        ...
        {
            // The caller is a script. It gets a sentence it can log, not a stack trace, and SIRS
            // stays up either way - a bad remote command must never take the show off air.
            return (500, $"SIRS could not do that: {ex.Message}");
        }

        // 409 rather than 400: the request was well formed, SIRS is simply not in a state where it
    ...
    /// <summary>Readable down a terminal, and what <c>SIRS.exe --status</c> prints.</summary>
    ...
    private string HelpText() =>
        $"""
         SIRS remote control.
           ...
           GET /status              what SIRS is doing, as text
         ...
         The same commands are on the command line: SIRS.exe --status, --live, --off, --title "...".
         """;
