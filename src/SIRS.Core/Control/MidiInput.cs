...
/// and forwards raw numbers. A MIDI desk is the one thing in SIRS that cannot be simulated, so the
...
public sealed class MidiInput : IDisposable
{
    ...
    public bool Start(string? deviceName)
    {
        lock (_lock)
        {
            ...
            if (index < 0)
            {
                Problem = $"SIRS could not find \"{deviceName}\". Is it plugged in?";
                return false;
            }
            ...
            catch (Exception ex)
            {
                ...
                Problem = $"SIRS could not open \"{deviceName}\": {ex.Message}";
                return false;
            }
        }
    }
