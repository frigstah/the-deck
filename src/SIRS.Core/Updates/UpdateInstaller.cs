...
/// <summary>
/// Downloads a release and replaces the running copy of SIRS with it.
/// <para>
/// This is the part that needed convincing. SIRS spent four phases deliberately <em>not</em> doing
...
public sealed class UpdateInstaller
{
    ...
    {
        Client.DefaultRequestHeaders.UserAgent.ParseAdd($"SIRS/{UpdateChecker.CurrentVersion}");
    }

    /// <summary>Where SIRS is installed — the folder that will be replaced.</summary>
    ...
    /// <summary>
    /// Whether SIRS can replace its own files. False for an install under Program Files without
    /// elevation, which is why the installer puts SIRS in the per-user location instead.
    /// </summary>
    public static bool CanInstallInPlace(out string reason)
    {
        ...
        {
            reason = $"SIRS cannot write to its own folder ({InstallDirectory}). " +
    ...
    public async Task<InstallResult> InstallAsync(ReleaseInfo release, CancellationToken cancellationToken = default)
    {
        ...
        {
            return new InstallResult(false, $"That download is {payload.Size / 1024 / 1024} MB, which is not credible for SIRS.");
        }
        ...
        try
        {
            ...
            {
                return new InstallResult(false,
                    "That release does not publish a checksum, so SIRS will not install it. " +
            ...
            if (!actual.Equals(expected, StringComparison.OrdinalIgnoreCase))
            {
                ...
                return new InstallResult(false,
                    "The download did not match its checksum, so SIRS threw it away. " +
            ...
            var stagedExe = Path.Combine(staged, "SIRS.exe");
            if (!File.Exists(stagedExe))
            {
                return new InstallResult(false, "The download did not contain SIRS.exe. Nothing has been changed.");
            }
            ...
            return new InstallResult(true, "Installing the update. SIRS will restart on its own.");
        }
        catch (Exception ex) when (ex is HttpRequestException or IOException or InvalidDataException
                                       or UnauthorizedAccessException or TaskCanceledException)
        {
            return new InstallResult(false, $"SIRS could not install that update: {ex.Message}");
        }
    }
    ...
    private async Task<string> DownloadAsync(ReleaseAsset asset, string path, CancellationToken cancellationToken)
    {
        ...
        if (total > MaxPayloadBytes) throw new IOException("The download is larger than SIRS will accept.");
        ...
        {
            read += count;
            if (read > MaxPayloadBytes) throw new IOException("The download grew beyond what SIRS will accept.");
    ...
    private static string RefusedMessage(string url) =>
        $"That release points somewhere SIRS will not download from ({url}). Nothing has been changed.";
