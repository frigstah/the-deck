...
public static class UpdateApplier
{
    ...
    /// start. Deliberately absent from the help text: it is SIRS talking to itself, not a command
    ...
    public static string? Apply(Request request)
    {
        ...
        if (!WaitForExit(request.WaitForProcessId))
        {
            ...
            return "SIRS did not close, so the update was not installed. " +
                   "Nothing has been changed. Close SIRS and try again.";
        }
        ...
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
        {
            ...
            return $"The update could not be finished: {ex.Message}\n\n" +
                   $"SIRS in {target} may be incomplete. Reinstall it from the release page.";
        }

        try
        {
            Process.Start(new ProcessStartInfo(Path.Combine(target, "SIRS.exe"))
            {
        ...
        {
            return $"The update was installed but SIRS would not restart: {ex.Message}";
        }
    }
