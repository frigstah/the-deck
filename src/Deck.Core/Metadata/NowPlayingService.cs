...
public sealed class NowPlayingService : IDisposable
{
    ...
    private void PollFile()
    {
        ...
        try
        {
            ...
            {
                SourceProblem = $"Deck cannot find {Path.GetFileName(FilePath)}. Check the file is still there.";
                return;
            }
        ...
        {
            SourceProblem = $"Deck could not read {Path.GetFileName(FilePath)}: {ex.Message}";
        }
    }
