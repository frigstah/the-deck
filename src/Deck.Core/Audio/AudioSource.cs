            ...
            else
            {
                ...
                var device = AudioDevices.ResolveOrDefault(deviceId, lookupKind)
                    ?? throw new AudioDeviceUnavailableException(
                        $"Deck could not find the device for {Name}. Choose a different one, or check it is plugged in and enabled in Windows.");
    ...
    private void OnRecordingStopped(object? sender, StoppedEventArgs e)
    {
        ...
        Failed?.Invoke(this, new CaptureFailedEventArgs(
            e.Exception is null
                ...
                : $"Deck lost {Name}. It may have been unplugged or taken over by another program.",
            e.Exception));
    }
