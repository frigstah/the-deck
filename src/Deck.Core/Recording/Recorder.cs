...
public sealed class Recorder : IDisposable
{
        ...
        lock (_lock)
        {
            ...
            catch (IOException ex)
            {
                ...
                Failed?.Invoke(this, new RecordingFailedEventArgs(
                    $"Deck had to stop recording: {ex.Message}. The file so far is saved at {path}."));
            }
        }
    }
    ...
    private void CheckDiskSpace()
    {
        ...
        if (free <= settings.MinimumFreeBytes)
        {
            ...
            StopInternal();
            Failed?.Invoke(this, new RecordingFailedEventArgs(
                $"Deck stopped recording because the drive is nearly full ({Describe(free)} left). " +
        ...
        if (!_lowSpaceWarned && free <= settings.LowSpaceWarningBytes)
        {
            ...
            LowDiskSpace?.Invoke(this, new RecordingFailedEventArgs(
                $"The drive holding your recordings has {Describe(free)} left. Deck will stop recording if it runs out."));
        }
    }
