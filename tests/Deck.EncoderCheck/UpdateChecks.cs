...
/// The self-updater (I9).
/// <para>
/// This is the one feature in Deck that can replace Deck, so the checks here are less about it
...
internal static class UpdateChecks
{
    public static int Run()
    {
        ...
        {
            var allowed = Allows("https://github.com/frigstah/the-deck/releases/download/v1/x.zip");
            ...
            // The whole point of pinning. Each of these is a way to make Deck run someone
            // else's code, and each has to be a flat no rather than a warning.
            Expect(!Allows("http://github.com/frigstah/the-deck/x.zip"), "plain http was allowed");
        ...
        {
            // Installing the portable zip over a normal install would drop deck-portable.txt into
            ...
            var release = new ReleaseInfo
            {
                ...
                Assets =
                [
                    new ReleaseAsset { Name = "Deck-1.3.0.42-setup.exe" },
                    new ReleaseAsset { Name = "Deck-1.3.0.42-portable-win-x64.zip" },
                    new ReleaseAsset { Name = "Deck-1.3.0.42-update-win-x64.zip" },
                    new ReleaseAsset { Name = "SHA256SUMS.txt" },
                ],
            };

            Expect(release.UpdatePayload?.Name == "Deck-1.3.0.42-update-win-x64.zip",
        ...
        {
            var request = UpdateApplier.Parse(["--apply-update", "--target", @"C:\Programs\Deck", "--wait", "4321"]);

            Expect(request is not null, "the update command was not recognised");
            Expect(request!.Target == @"C:\Programs\Deck", $"target came back as {request.Target}");
            Expect(request.WaitForProcessId == 4321, $"pid came back as {request.WaitForProcessId}");

            // An ordinary launch, and the other command lines Deck already answers, must never be
        ...
        failures += Check("a payload that is not Deck is refused", () =>
        {
            ...
            try
            {
                ...
                File.WriteAllText(Path.Combine(content, "something-else.exe"), "not Deck");
                ...
                Expect(!File.Exists(Path.Combine(staged, "Deck.exe")),
                    "a payload without Deck.exe looked complete");
            }
            finally
            {
        ...
        failures += Check("the portable marker is never carried across an update", () =>
        {
            ...
            Expect(marker == "deck-portable.txt",
        ...
        failures += Check("staging happens outside the install folder", () =>
        {
            ...
            Expect(staging.Contains("Deck", StringComparison.OrdinalIgnoreCase),
        ...
        failures += Check("a release with no checksum is refused", () =>
        {
            var release = new ReleaseInfo
            {
                ...
                {
                    Name = "Deck-9.9.9.9-update-win-x64.zip",
                    Url = "https://github.com/frigstah/the-deck/releases/download/v9.9.9.9/Deck-9.9.9.9-update-win-x64.zip",
                }],
            };
