...
/// <para>
/// Only what SIRS produces is supported - fixed predictors, Rice partitions, 16-bit samples. An LPC
...
internal sealed class FlacReader(byte[] frame)
{
    ...
    public int[][] Decode(int bitsPerSample)
    {
        ...
        if (variableBlocking) throw new Exception("variable blocking was used; SIRS should always use fixed");
    ...
    private int[] ReadSubframe(int bitsPerSample)
    {
        ...
        switch (type)
        {
            ...
            case >= 32:
                throw new Exception($"subframe type {type} is an LPC subframe; SIRS should only emit fixed predictors");
