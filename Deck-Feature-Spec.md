...
### The gap Deck fills
Both are *encoder-shaped*: they assume you already know what a mountpoint is. Nobody has built the "it just works, and it tells you if it doesn't" version. Deck's whole reason to exist is **first-run success in under 3 minutes** and **never wondering whether you sound OK**.
...
| B9 | Spectrum / correlation display | P3 — **built** | Collapsed by default. The correlation half turned out to be the valuable one: nothing else in Deck can see a miswired cable |
...
| G5 | Record-only mode (no server needed) | P1 | Lets people use Deck as a plain recorder — good on-ramp |
...
Keeping these out is what makes Deck simple. Say no on purpose:
...
*Alternative if you want a leaner binary:* C++ with Qt (what BUTT uses) — better raw control, significantly slower to build the polished UI that is Deck's entire point. I'd take the C# speed advantage.
...
Hosting means scanning plugins, opening a third party's editor window inside Deck, and surviving a
...
The two things that must be excellent, because they're the reason to choose Deck over free BUTT: **the sound test flow (§3.2)** and **server setup (§3.3)**. Spend disproportionate effort there.
